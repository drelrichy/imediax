...
export default  function Newpass({ action }) {

  //----- usefull if using search string
const searchParams = useSearchParams();

const token =(action)? action : searchParams.get("token");

 //if (token) action = token;

  useEffect(() => {
    const init = async () => {
      const { Datepicker, Input, initTE, Collapse, Ripple } = await import(
        "tw-elements"
      );
  ...
  //console.log("in the component --> ", action);

 // if (action) const token = action
...
//console.log("indisder ", feederror);
   ...
   // if (feederror)  setError(feederror);
    ...
    if (password1 != password) {
      setError("Password entered did not match the verification");
      return;
    }

    try {
      setMessage("Processing please wait..");
      const res = await fetch(HOST + "api/newpass", {
        method: "POST",
        headers: {
      ...
      if (feedMessage) {
        ...
        console.log('will go to '+HOST+"login")
        setTimeout(() => {
      console.log('going to '+HOST+"login")
       ...
       // console.log(' { feedbackError }', feedError);
...
console.log("------------------------------------------", token)
 ...
 const checktoken = await fetch(HOST+"api/passtoken", {
 ...
 const feedback = await checktoken.json();
  const feedMessage = feedback.message;
  const feedError = feedback.error;

    if (feedError) {
       
     // feederror = feedback.error;
      console.log("!!!feed bck check 001",feedback.error);
...
if(runCheck)handleCheck(token,Event);
            ...
            <div className="mb-12 md:mb-0 md:w-8/12 lg:w-5/12 xl:w-5/12">
              <form onSubmit={handleSubmit}>
                ...
                <div className="flex flex-row items-center justify-center lg:justify-start">
                  <p className="mb-0 mr-4 text-lg">Password Recovery</p>
                ...
                {message && (
                    <div     id="mm" key="mm" className="bg-blue-500 text-white w-fit text-sm py-1 px-3 rounded-md mt-2">
                      {message}
                    </div>
                  )}
                  {error && (
                    <div     id="ee" key="ee" className="bg-red-500 text-white w-fit text-sm py-1 px-3 rounded-md mt-2">
                      {error}
                    </div>
                  )}
                {
                ...
                <div className="text-center lg:text-left">
                  ...
                  {error && (
                    <Link className="text-sm mt-3 text-left" href={"/forgot"}>
                      forgot password?{" "}
